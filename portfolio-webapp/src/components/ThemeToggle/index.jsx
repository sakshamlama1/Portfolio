...
const ThemeToggle = ({ className = "", mobile = false }) => {
  ...
  return (
    <button
      ...
      className={`relative inline-flex items-center cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-full transition ${className}`}
      style={{
    ...
    >
      ...
      >
        ...
        >
          <Sun size={mobile ? 18 : 20} />
        </span>
      </span>

      {/* Thumb */}
      <span
        className={`absolute top-1/2 transform -translate-y-1/2 rounded-full bg-white shadow-md transition-transform ${
